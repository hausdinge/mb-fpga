...
input fixedpoint::number scale, input fixedpoint::number angle, output fixedpoint::number sin, output fixedpoint::number cos, output logic out_valid);
  ...
  logic [23:0] sin_sign = 0;
  logic [15:0] cos_sign = 0;
  logic [23:0] out_valid_reg = 0;
  ...
  always_comb begin
    ...
    cos = 1'b0;
    out_valid = 1'b0;
    if(out_valid_reg[23]) begin
      sin = sin_sign[23] ? ~y[widht-1] + 1'b1 : y[widht-1];
      cos = cos_sign[15] ? ~x[widht-1] + 1'b1 : x[widht-1];
      out_valid = 1'b1;
    end
  end
endmodule
